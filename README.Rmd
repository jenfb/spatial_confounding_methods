...
For illustration, here we generate a single dataset from one set of parameter values. To save computation time for this illustration we generate a dataset of size `r n_obs` (in the paper we used 2500). The parameter values we use are defined here:
```{r}
params <- params_all %>%
